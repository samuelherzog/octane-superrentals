...
module('Unit | Service | map-element', function(hooks) {
  ...
  test(
    'should create a new map if one isnt cached for location', 
    async function (assert) {

    assert.expect(5);
    let stubMapElementService = {
    ...
    let mapElementService = this.owner.factoryFor('service:map-element')
      .create({
        map: stubMapElementService,
        geocode: stubGeocodeService
      });

    let element = await mapElementService.getMapElement('San Francisco');
  ...
  test(
    'should use existing map if one is cached for location', 
    async function (assert) {
    ...
    let mapService = this.owner.factoryFor('service:map-element')
      .create({
